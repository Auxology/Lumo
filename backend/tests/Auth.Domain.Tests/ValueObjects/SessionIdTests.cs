using Auth.Domain.ValueObjects;

using FluentAssertions;

using SharedKernel;

namespace Auth.Domain.Tests.ValueObjects;

public sealed class SessionIdTests
{
    private const string ValidSessionId = "sid_01JGX123456789012345678901";
    private const string Prefix = "sid_";
    private const int ExpectedLength = 30;

    [Fact]
    public void From_WithValidId_ShouldReturnSuccess()
    {
        Outcome<SessionId> outcome = SessionId.From(ValidSessionId);

        outcome.IsSuccess.Should().BeTrue();
        outcome.Value.Value.Should().Be(ValidSessionId);
    }

    [Theory]
    [InlineData("")]
    [InlineData(" ")]
    [InlineData(null)]
    public void From_WithEmptyOrWhitespace_ShouldReturnFailure(string? value)
    {
        Outcome<SessionId> outcome = SessionId.From(value);

        outcome.IsFailure.Should().BeTrue();
        outcome.Fault.Title.Should().Be("SessionId.Required");
    }

    [Theory]
    [InlineData("invalid_id")]
    [InlineData("sid_short")]
    [InlineData("sid_01JGX12345678901234567890")] // 29 chars - too short
    [InlineData("sid_01JGX1234567890123456789012")] // 31 chars - too long
    [InlineData("xxx_01JGX12345678901234567890")] // wrong prefix
    public void From_WithInvalidFormat_ShouldReturnFailure(string value)
    {
        Outcome<SessionId> outcome = SessionId.From(value);

        outcome.IsFailure.Should().BeTrue();
        outcome.Fault.Title.Should().Be("SessionId.InvalidFormat");
    }

    [Fact]
    public void UnsafeFrom_ShouldCreateWithoutValidation()
    {
        string invalidId = "invalid";

        SessionId result = SessionId.UnsafeFrom(invalidId);

        result.Value.Should().Be(invalidId);
    }

    [Fact]
    public void ToString_ShouldReturnValue()
    {
        SessionId id = SessionId.UnsafeFrom(ValidSessionId);

        string result = id.ToString();

        result.Should().Be(ValidSessionId);
    }

    [Fact]
    public void IsEmpty_WhenEmpty_ShouldReturnTrue()
    {
        SessionId id = SessionId.UnsafeFrom(string.Empty);

        id.IsEmpty.Should().BeTrue();
    }

    [Fact]
    public void IsEmpty_WhenNotEmpty_ShouldReturnFalse()
    {
        SessionId id = SessionId.UnsafeFrom(ValidSessionId);

        id.IsEmpty.Should().BeFalse();
    }

    [Fact]
    public void PrefixValue_ShouldReturnPrefix()
    {
        SessionId.PrefixValue.Should().Be(Prefix);
    }

    [Fact]
    public void Length_ShouldReturnExpectedLength()
    {
        SessionId.Length.Should().Be(ExpectedLength);
    }

    [Fact]
    public void Equality_WithSameValue_ShouldBeEqual()
    {
        SessionId id1 = SessionId.UnsafeFrom(ValidSessionId);
        SessionId id2 = SessionId.UnsafeFrom(ValidSessionId);

        id1.Should().Be(id2);
    }

    [Fact]
    public void Equality_WithDifferentValue_ShouldNotBeEqual()
    {
        SessionId id1 = SessionId.UnsafeFrom("sid_01JGX123456789012345678901");
        SessionId id2 = SessionId.UnsafeFrom("sid_01JGX123456789012345678902");

        id1.Should().NotBe(id2);
    }

    [Fact]
    public void ValidId_ShouldHaveCorrectLength()
    {
        ValidSessionId.Length.Should().Be(ExpectedLength);
    }

    [Fact]
    public void ValidId_ShouldStartWithPrefix()
    {
        ValidSessionId.Should().StartWith(Prefix);
    }
}